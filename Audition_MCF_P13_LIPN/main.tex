...
\begin{frame}{Enseignement : expérience}
	\begin{itemize}
		\item CI $\approx$ cours
		\item Prog C : programmation ``en direct'', faire des erreurs exprès
		\item Proba, stat, combinatoire : rappels des notions de base, réexpliquer les cours
		\item Algo des arbres : élaboration des tests, comparer le niveau avec d'autres groupes, réviser les copies / grille d'évaluation avec les collègues
		\item groupe de lecture (MPI 2026) : élaborer le programme
	\end{itemize}
\end{frame}
...
\begin{frame}{Surfaces à petits carreaux}
	\vspace{5mm}
	\begin{minipage}{0.4\linewidth}
		\centering
		\begin{figure}
			\centering
			\includegraphics[width=0.35\linewidth]{squares-french.pdf}
			\caption{\small Règle de recollement :\\ $H \leftrightarrow B$, $G \leftrightarrow D$}
		\end{figure}
	\end{minipage}%
\begin{minipage}{0.58\linewidth}
	\vspace{-7mm}
	\begin{figure}
		\includegraphics[width=0.55\linewidth]{surface.eps}
		\end{figure}
\end{minipage}
\pause 
	\centering
	\small Décomposition : cylindres + carte métrique
	\begin{figure}
		\centering
		\includegraphics[width=0.6\linewidth]{cyl-decomp.pdf}
	\end{figure}
\end{frame}

\begin{frame}{Cartes combinatoires (métriques)}
	...
	\begin{figure}
		\begin{overprint}
			\onslide<1> 
			\centering \includegraphics[width=0.7\textwidth]{def-rg-1.pdf}
			\onslide<2> 
			\centering \includegraphics[width=0.7\textwidth]{def-rg-2.pdf}
			\onslide<3-> 
			\centering \includegraphics[width=0.7\textwidth]{def-rg-3.pdf}
		\end{overprint}
	\end{figure}
	
	\pause \pause \pause 
	\bigskip
	Métrique sur $G$  = fonction $E(G) \rightarrow \Rm_{>0}$
	
\end{frame}

\begin{frame}{Paramétrer les cartes métriques ?}
	\begin{itemize}
		\item Métriques sur $G$ $\leftrightarrow$ cône $\Rm_{>0}^{E(G)}$
		\pause
		\item longueur de $e$ est zéro  $\leftrightarrow$ contraction de $e$
		\pause
		\item recollement naturel des cônes (``flips'') :
	\end{itemize}
	\begin{figure}
		\centering
		\includegraphics[width=0.5\textwidth]{recollement-mgn-comb-2.pdf}
	\end{figure}
	\pause
	$\leadsto$ complexe polyédrique $\mathcal{M}_{g,n}^{comb}$ (``espace de modules combinatoire'')
%    \begin{figure}
%
%        \centering
...
\begin{frame}{Comptage de cartes métriques}

\textbf{Problème} : compter les cartes de genre $g$ avec $n$ faces de longueurs $L_1,\ldots,L_n$.
\bigskip
%\pause
\pause

\textbf{Théorème (Kontsevich '92).} Cartes \alert{trivalents} $\Rightarrow$ \alert{polynôme} en $L_i$ dont les coefficients sont les nombres d'intersection $\int_{\overline{\Mc}_{g,n}} \psi_1^{d_1}\cdots \psi_n^{d_n}$.
\bigskip
\pause

\textbf{Théorèmes.} Fonctions de comptage \alert{polynomiales} explicites :\\
\begin{itemize}
	\item \textbf{(Y. '23)} cartes \alert{face-bicolores à un sommet}.
	\item \textbf{(Y. '24)} Comptage pondéré des cartes \alert{face-bicolores à $\ge 2$ sommets}.
	\item \textbf{(Duryev-Goujard-Y. '25)} cartes aux \alert{degrés de sommets impaires fixés}.
\end{itemize}    
\bigskip

\pause
$\leadsto$ \textbf{Applications :} géométrie des SPC aléatoires, calcul des volumes de Masur--Veech, leurs analyse asymptotique, raffinement des résultats algébriques...
...
\begin{frame}{Outils : combinatoire bijective}
    ...
    \textbf{(Y. '23)} : \alert{bijection de Chapuy-Féray-Fusy '13} entre les cartes à une face et les arbres planaires décorés.

    \textbf{(Y. '24)} : \alert{bijection de Bernardi '07} entre les cartes planaires munies d'un arbre couvrant et les paires d'arbres planaires.

    \textbf{(D-G-Y. '25)} : \alert{bijection à la Pr\"{u}fer} pour certaines structures arborescentes.
...
\begin{frame}{Projet de recherche / intégration}
	
    \begin{figure}
	\centering
	\def\svgwidth{0.95\textwidth}
	\footnotesize
	\input{circles-projet.pdf_tex}
\end{figure}
\end{frame}
...
%	\item Axel Bacher, Frédérique Bassino (combinatoire énumérative).
%\end{itemize}
%\end{frame}
...
\begin{frame}{Piecewise polynomiality}
...
\begin{frame}{Cylinder contributions in $\Hc(2g-2)$}
	...
	\begin{itemize}
		...
		\item setting $u=1$ we recover the result of Sauvaget.
		\item Faber-Pandharipande 2000: $\left(\frac{t/2}{\sin(t/2)}\right)^{u+1}$ is the generating function of Hodge integrals $\int_{\overline{\mathcal{M}}_{g,1}} \lambda_{g-i} \psi_1^{2g-2+i}$
	\end{itemize}
\end{frame}

\begin{frame}{Many-vertex face-bicolored graphs}
...
\begin{frame}{Computing degeneration coefficients on the walls}
    ...
    (k+l-2)! = p^{b_1,\ldots,b_n}_{w_1,\ldots,w_n} + \sum_{t=2}^n \frac{ (k+l-2)_{t-2}}{t!} \sum_{I_1, \ldots I_t} \prod_{j=1}^t \left(\sum_{i\in I_j} (b_i+w_i) - 1\right) p^{b_{I_j}}_{w_{I_j}},
    \end{equation*}
\end{frame}
...
\end{theorem}
 ...
 $u=1$ : formula from Chen, M\"{o}ller, Sauvaget, Zagier.
\end{frame}
...
%    
%        \onslide<1-2> 
%        \centering \includegraphics[width=0.8\textwidth]{def-rg-1.pdf}
%        \onslide<3> 
%        \centering \includegraphics[width=0.8\textwidth]{def-rg-2.pdf}
%        \onslide<4-> 
%        \centering \includegraphics[width=0.8\textwidth]{def-rg-3.pdf}
%    \end{overprint}
%\end{figure}
%
%\pause
...
%        \item $g=0$: elementary count of metric plane trees; %\includegraphics[width=0.3\textwidth]{tree-4.pdf}
%    \end{itemize}
%    \begin{figure}
%        \centering
%        \vspace{-8mm}
...
% %             \centering
% %             \includegraphics[height=0.4\textheight]{st-example-2.pdf}
% %             \hspace{2cm}\includegraphics[height=0.4\textheight]{ribbon_graph_to_st-0.pdf}
% %             \onslide<7->
% %             \centering
% %             \includegraphics[height=0.4\textheight]{st-example-2.pdf}
% %             \hspace{2cm}\includegraphics[height=0.4\textheight]{ribbon_graph_to_st-1.pdf}
